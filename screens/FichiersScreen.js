...
export default ({ navigation }) => {
  ...
  const [refreshcompt, setRefreshCompt] = useState(0);
  ...
  const getCategories = async () => {
    const data = await fetch(nuage + "api/categories?page=1");
    const dataJSON = await data.json();

    setCategories(dataJSON["hydra:member"]);
  };
  ...
  const isFichierAffiche = (item) => {
    if (item.categories.length == 0) return true;

    for (let i = 0; i < item.categories.length; i++) {
      for (let j = 0; j < categores.length; j++) {
        if (categores[j]["@id"] == item.categories[i]) {
          if (categores[j].themes.includes(theme["@id"])) return true;
        }
      }
    }
  };
    ...
    <View style={styles.body}>
      ...
      <View style={styles.blueContainer}>
        <View style={styles.row}>
          <View style={{ ...styles.ecart, flexDirection: "row" }}>
            <Text style={styles.blueTitre}>Fichiers</Text>
            <View style={{ flexDirection: "column" }}>
              <Button
                title={"Thème : " + theme.libelle}
                onPress={() => {
